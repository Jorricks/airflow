...
def upgrade():
    ...
    with op.batch_alter_table('dag_run') as batch_op:
        if conn.dialect.name == "mysql":
            batch_op.add_column(sa.Column('user_notes', sa.Text(length=1000), nullable=True))
        else:
            batch_op.add_column(sa.Column('user_notes', sa.String(length=1000), nullable=True))
